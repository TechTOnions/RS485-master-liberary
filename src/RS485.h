...
class RS485 {
  ...
  public:
    //constructor
#if (defined(__AVR__) || defined(ESP8266)) && defined(USE_SW_SERIAL)
    RS485(SoftwareSerial *ser); // Constructor when using SoftwareSerial
    RS485(SoftwareSerial *ser, int8_t enPin);
#endif
    RS485(HardwareSerial *ser); // Constructor when using HardwareSerial
    RS485(HardwareSerial *ser , int8_t enPin);
    // Method /functions
