...
/*!
  @file RS485.cpp
  @mainpage RS485 master liberary
  @section intro Introduction
  RS485 master liberary which supports RS485 modbus communication.
...
RS485::RS485(SoftwareSerial*ser) {
  common_init();
  SwSerial = ser;
}

RS485::RS485(SoftwareSerial*ser, int8_t enPin) {       // if using enable pin for RS485 and softwareserial
  common_init();
  SwSerial = ser;
  if (enPin >= 0)
  {
...
RS485::RS485(HardwareSerial *ser) {
  common_init();
  HwSerial = ser; // ...override HwSerial with value passed.

}

RS485::RS485(HardwareSerial *ser, int8_t enPin) {     // if using enable pin for RS485 and hardwareSerial.
  common_init();
...
float RS485::read_holding_reg(byte slave_id, unsigned int address, byte data_type, byte data_order, float gain)  // uses FC = 3
{
  received_data = 0;
...
float RS485::read_input_reg(byte slave_id, unsigned int address, byte data_type, byte data_order, float gain)  // uses FC = 4
{
  received_data = 0;
  if (data_type == 1 || data_type == 5)
  {
...
boolean RS485::write_single_reg(byte slave_id, unsigned int address,  int16_t writeValue)
{
  read_reg(slave_id, 6, address, writeValue , 2, 2, 2);
  return _write_data;
}

boolean RS485::write_single_coil(byte slave_id, unsigned int address, unsigned int writeValue)
{
  read_reg(slave_id, 5, address, writeValue , 2, 2, 2);
  return _write_data;
}

void RS485::read_reg(byte slave_id, byte function_code, unsigned int address, int16_t readUpto,  byte data_type, byte data_order, float gain)
{
...
unsigned int RS485::CRC(byte* buf, byte len, byte chk)  // for generating and validating CRC
{
  _crc = 0xffff;
  _crcPos = 0;
